...
module.exports = (eleventyConfig) => {
    ...
    eleventyConfig.addCollection("people", (collection) => {
        return collection.getFilteredByGlob("_team/*.md").sort((a, b) => {
    ...
    eleventyConfig.addFilter("commission", (c) => {
        const re = /[^\/]+$/g;
    ...
    eleventyConfig.addFilter("madeWith", (collection) => {
        const sortedCollection = collection.sort((a, b) => {
            return Number.parseInt(a.year) > Number.parseInt(b.year)
                ? 1
                : Number.parseInt(b.year) > Number.parseInt(a.year)
                  ? -1
                  : 0;
        });
        let res = '<ul class="madewith-block">';
        for (const a of sortedCollection) {
            res += `<li class="madewith-item">
